...
def evaluate(args, model, features, tag="test"):
    ...
    freqP = preds[:, :5].sum()
    freqT = golds[:, :5].sum()
    fp = freqTP / freqP if freqP != 0 else 0
    fr = freqTP / freqT if freqT != 0 else 0
    f_f = 2 * fp * fr / \
        (fp + fr) if fp + fr > 0 else 0

    ltTP = TPs[:, 5:].sum()
    ltP = preds[:, 5:].sum()
    ltT = golds[:, 5:].sum()
    lp = ltTP / ltP if ltP != 0 else 0
    lr = ltTP / ltT if ltT != 0 else 0
    l_f = 2 * lp * lr / \
        (lp + lr) if lp + lr > 0 else 0
...
def main():
    ...
    parser.add_argument('--aug', type=str, default='no_aug', help='aug type: no_aug/pos_aug/all_aug')
    parser.add_argument('--dropout_rate', type=float, default=0.1, help='dropout_rate')
    parser.add_argument("--use_mixup", type=int, default=0, help='1 means use mixup, 0 means not use')
    parser.add_argument('--mixup_alpha', default=0.3, type=float, help='mixup_alpha')
    parser.add_argument('--mixup_rate', default=0.1, type=float, help='mixup_rate')
    parser.add_argument('--mixup_type', default='r_emb', type=str, help='r_emb/e_emb')   
    parser.add_argument('--la', default=10, type=float, help='lambda')
    args = parser.parse_args()

    if not os.path.exists(args.save_path):
        os.mkdir(args.save_path)
    file_name = "{}_{}_{}_e_{}_la_{}_aug_{}_dropout_rate_{}_use_mixup_{}_mixup_alpha_{}_mixup_rate_{}_mixup_type_{}_seed_{}".format(
